...
class StudyDataset(Dataset):
    def __init__(
        ...
        for _, row in df.iterrows():
            if os.path.exists(os.path.join(data_root, row['study_uid'])):
                self.studies.append((os.path.join(data_root, row['study_uid']), row[list(CLASSNAMES)].astype(int).tolist()))
    ...
    def __getitem__(self, idx):
        ...
        image = []
        for scan in [
            os.path.join(study, p, "img.pt")
            for p in os.listdir(study)
            if os.path.isfile(os.path.join(study, p, "img.pt"))
        ]:            
