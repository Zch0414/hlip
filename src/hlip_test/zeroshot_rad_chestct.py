...
from hlip.zeroshot_metadata_rad_chestct import CLASSNAMES, ORGANS, TEMPLATES, PROMPTS
...
def get_args_parser():
    ...
    parser.add_argument('--rad-chestct-root', default='/data/rad_chestct/')
    parser.add_argument('--input-filename', default='../../data/rad_chestct/files/rad_chestct_labels.csv', type=str)
...
def get_data(args, preprocess_fn=None):
    ...
    dataset = ZeroShotDataset(
        args.rad_chestct_root, args.input_filename, args.input_info,
        preprocess_fn
    )
...
def zero_shot(model, tokenizer, dataloader, args):
    ...
    with torch.inference_mode():
        ...
        results_dir = f"./results/rad_chestct/{args.model}/"
        ...
        # merge "Coronary artery wall calcification" and "Arterial wall calcification"
        preds["Calcification"] = []
        for i, (a, b) in enumerate(zip(preds["Coronary artery wall calcification"], preds["Arterial wall calcification"])):
            if a or b:
                preds["Calcification"].append(1)
            else:
                preds["Calcification"].append(0)
        logits["Calcification"] = []
        for i, (a, b) in enumerate(zip(logits["Coronary artery wall calcification"], logits["Arterial wall calcification"])):
            if a > 0.5 or b > 0.5:
                logits["Calcification"].append(max(a, b))
            else:
                logits["Calcification"].append(min(a, b))
        labels["Calcification"] = labels["Coronary artery wall calcification"] # also same as labels["Arterial wall calcification"]

        # reset CLASSNAMES
        classnames = []
        for key in CLASSNAMES:
            if key == "Coronary artery wall calcification" or key == "Arterial wall calcification":
                continue
            classnames.append(key)
        classnames.append("Calcification")
